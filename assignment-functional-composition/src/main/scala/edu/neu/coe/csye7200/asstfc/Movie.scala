...
object Movie extends App {
  ...
  object MoviesProtocol extends DefaultJsonProtocol {
    ...
    implicit val rateingFormat = jsonFormat2(Rating.apply)
    implicit val reviewsFormat = jsonFormat7(Reviews.apply)
    implicit val pricipleFormat = jsonFormat2(Principal.apply)

    implicit val movieFormat = jsonFormat11(Movie.apply)
    implicit val movieSeq : RootJsonFormat[Moives]= jsonFormat1(Moives)
  }
  ...
  private def fromJson(string: String) = string.parseJson.convertTo[Seq[Movie]]
  def testSerializationAndDeserialization(ms: Seq[Movie]): Boolean = {
    // 5 points
    // TO BE IMPLEMENTED
    val source = ms.toJson.toString
    val deserialize = fromJson(source)
//    println("是否相等:",deserialize==(ms))
    deserialize.equals(ms)
  }
