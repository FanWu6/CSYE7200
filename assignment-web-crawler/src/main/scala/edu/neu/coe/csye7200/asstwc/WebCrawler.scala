...
object WebCrawler extends App {
  ...
  def wget(u: URL): Future[Seq[URL]] = {
    ...
    // 9 points.
    for (str <- getURLContent(u); link <- MonadOps.asFuture(getLinks(str))) yield link // TO BE IMPLEMENTED
  }
  ...
  def wget(us: Seq[URL]): Future[Seq[Either[Throwable, Seq[URL]]]] = {
    ...
    // TO BE IMPLEMENTED
//    val value: Seq[Future[Seq[URL]]] = for (u <- us2) yield wget(u)
//    Future.sequence(MonadOps(value))
    val usf2: Seq[Future[Seq[URL]]] = for (u1 <- us2) yield wget(u1)
    val usfr: Seq[Future[Either[Throwable, Seq[URL]]]] = for (us <- usf2) yield MonadOps.sequence(us)
    Future.sequence(usfr)
  }
